...
public class RegexHelper {
	/*
	 * 1. 싱글톤 2. boolean isValue(String str), str : 검사할 문자열, 공백, null체크 (펄스리턴) 3.
	 * boolean isNum(String str), str : 검사할 문자열, 숫자형식 검증체크 4. boolean isEng(String
	 * str), str : 검사할 문자열, 영문인지 검증체크 5. boolean isKor(String str), str : 검사할 문자열,
	 * 한글인지 검증체크 6. boolean isEngNum(String str), str : 검사할 문자열, 영문/숫자인지 검증체크 7.
	 * boolean isKorNum(String str), str : 검사할 문자열, 한글/숫자인지 검증체크 8. boolean
	 * isEmail(String str), str : 검사할 문자열, email인지 검증체크 9. boolean
	 * isCellPhone(String str), str : 검사할 문자열, 휴대전화번호인지 검증체크
	 * ----------------------------------------------------------------------------
	 * main() class에서 해당 메서드 사용
	 */

	// -------------------싱글톤 객체 생성 시작-------------------
	private static RegexHelper current; // 싱글톤 객체 생성

	public static RegexHelper getInstance() {
		if (current == null) { // current에 값이 없다면
			current = new RegexHelper(); // regexHelper의 값을 current에 넣어주고
		}

		return current; // 그렇지 않다면 current의 값을 반환해 줄 거야
